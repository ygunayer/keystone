...
function mount(mountPath, parentApp, events) {
  debug('mounting');
	...
	var onConnectionFailure = function(err) {
		if (keystone.get('logger')) {
			console.log('------------------------------------------------');
	...
	if (this.get('mongo replica set')){
		...
		this.mongoose.connect(replica, options).then(onConnectionOpen).catch(onConnectionFailure);
	} else {
		var connectOptions = this.get('mongo options');
		var mongoHost = this.get('mongo');
		debug('connecting to mongo', mongoHost, 'with options', connectOptions);
		this.mongoose.connect(mongoHost, connectOptions).then(onConnectionOpen).catch(onConnectionFailure);
	}
}
